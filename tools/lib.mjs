// Shared helpers for the add-on harness (tools/addon-check.mjs).
...
export const PROFILES = {
    ...
    /* The official Jellyfin for Android app is a WebView wrapper around the server's own web
     * client, so with its "Video player type" set to "Web player" it runs exactly this code.
     * Sideloaded on an Android TV box it is driven by a remote, hence the TV layout and no
     * touch. The closest this harness can get to that box without one on the desk. */
    'android-webview': {
        viewport: { width: 1920, height: 1080 },
        layout: 'tv',
        userAgent: 'Mozilla/5.0 (Linux; Android 14; BRAVIA 4K Build/UP1A.231005.007; wv)'
...
/** Reads CSS to inject from a comma-separated list of paths, relative to the repo root. */
...
export async function newContext(browser, profileName, { injectCss = '', injectJs = '' } = {}) {
    ...
    const context = await browser.newContext({
        ...
        isMobile: !!p.mobile,
        hasTouch: !!p.mobile,
        userAgent: p.userAgent || (p.mobile ? MOBILE_UA : undefined),
        locale: 'pl-PL'
    });
