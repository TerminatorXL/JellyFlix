...
public class ScriptInjectionStartupFilter : IStartupFilter
{
    ...
    /// <summary>Counters behind GET /EpisodePicker/status, so a remote install can be diagnosed with one URL.</summary>
    ...
    {
        _logger?.LogInformation("Episode Picker: index.html middleware installed");
    ...
    private static async Task InjectAsync(HttpContext context, RequestDelegate next)
    {
         ...
         * ../EpisodePicker/addon.js resolves to /EpisodePicker/addon.js — and under /jellyfin/web/ it
         * resolves to /jellyfin/EpisodePicker/addon.js, which is exactly right.
        ...
        var tag = $"\n<!-- Episode Picker -->\n<script defer src=\"../EpisodePicker/addon.js?v={Stamp(plugin)}\"></script>\n";
        ...
        if (Injections == 1)
        {
            _logger?.LogInformation("Episode Picker: injected the add-on script tag into {Path}", context.Request.Path);
        }
    ...
    private static void Record(string outcome)
    {
        if (!string.Equals(LastOutcome, outcome, StringComparison.Ordinal))
        {
            ...
            {
                _logger?.LogWarning("Episode Picker: {Outcome}", outcome);
            }
        }
    }
