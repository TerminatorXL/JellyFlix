...
using Jellyfin.Plugin.EpisodePicker.Services;
...
[ApiController]
[Route("EpisodePicker")]
public class EpisodePickerController : ControllerBase
{
    private const string ResourceName = "Jellyfin.Plugin.EpisodePicker.addon.js";
    ...
    public ActionResult GetAddonScript()
    {
        ...
        {
            return Content("/* Episode Picker is disabled in the plugin settings. */",
                "application/javascript; charset=utf-8");
        }

        var script = new StringBuilder()
            .Append("/* Episode Picker ")
            .Append(plugin.Version?.ToString() ?? "?")
            .Append(" — settings from Dashboard > Plugins > Episode Picker */\n")
    ...
    private static string BuildConfigJson(PluginConfiguration config) =>
        ...
        {
            ["debug"] = config.Debug,
            ["episodePicker"] = new Dictionary<string, object>(StringComparer.Ordinal)
            {
                ["enabled"] = config.PickerEnabled,
                ["thumbnails"] = config.Thumbnails,
                ["showOverview"] = config.ShowOverview,
                ["overviewChars"] = Math.Clamp(config.OverviewChars, 0, 2000)
            }
        });
}
