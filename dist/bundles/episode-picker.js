...
/*! Generated from addons/ (8 modules, 1 stylesheets). Build dbc137914969.
 ...
 *
 * `dbc137914969` is a content hash of this exact bundle, stamped in by build-addon.mjs. That turns the
...
if (window.JellyFlixAddon && window.JellyFlixAddon.modules) {
    ...
    if (running.build === 'dbc137914969') {
        return;                                    // exact duplicate — the running copy is this copy
    }

    if (running.bundle === 'episode-picker') {
        ...
        if (typeof running.stop !== 'function') {
            console.warn('[JellyFlix] another build of "episode-picker" is running and cannot be stopped;'
                + ' this copy (dbc137914969) stands down. Remove one of the two installs.');
            return;
        }
        console.warn('[JellyFlix] replacing build', running.build, 'with dbc137914969');
        try {
            running.stop();
        } catch (err) {
        ...
        console.warn('[JellyFlix] an unrecognised add-on is already running; "episode-picker" stands down.');
        return;
    }
}
...
if (JFX_JOINED) {
    ...
    JFX.util.log('joined the running framework as bundle "episode-picker"');
